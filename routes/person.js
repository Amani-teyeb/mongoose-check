...
router.post('/added', async(req,res)=>{
    try {
        await Person.insertMany([
            {name:"houcine", "age": 56, email: "houcine@gmail.com", favoriteFoods:["makrouna", "jelbene"]},
            {name:"saliha", "age": 46, email: "saliha@gmail.com", favoriteFoods:["kousksi", "jelbene"]}
        ]);
        res.status(200).send("persons added");
    } 
    catch (error) {
       res.status(500).send("imp to add many persons")
    }
});
...
router.put('/:Id', async(req,res)=>{
  try {
      const id = req.params.Id;
      const person= await Person.findOneAndUpdate({_id:id}, {$set: {...req.body}});
      res.status(200).send({ msg: "person updated", person });
    } catch (error) {
        res.status(500).send("fail to update person"); 
  }
